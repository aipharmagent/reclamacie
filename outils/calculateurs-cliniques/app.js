...
const defs = {
  pedi: {
    label: 'Pédiatrique',
    fields: [
      {id:'poidsPair',label:'Poids',type:'weightPair'},
  ...
  abx_ped: {
    ...
    run:v=>{
      const kg=getWeightKg(v), prises=Math.max(1,v.prises||1);
    ...
    run:v=>{
      ...
      if(varId){
        if(t23){
          ...
          else if(i<=3.39)suggestion='INR suprathérapeutique — continuer même dose INR dans 2–4 semaines';
          ...
          else if(i<=3.99)suggestion='INR suprathérapeutique — continuer même dose INR dans 2–4 semaines';
        ...
        if(t23){
          ...
          else if(i<=3.79){suggestion='INR suprathérapeutique Diminuer dose de 5%';pct='-5%';}
      ...
      const freq=extractFollowup(suggestion),nextDate=addFromToday(freq.n,freq.unit);
  ...
  custom: {
    ...
    run:v=>{
      ...
      if(v.concMg&&v.concMl){
        ...
        const mlDose=dosePer/(v.concMg/Math.max(0.0001,v.concMl));
...
function extractFollowup(s){ const t=s.toLowerCase(); const m=t.match(/(\d+)\s*(?:–|-)?\s*(\d+)?\s*(semaine|semaines|jour|jours)/); if(!m) return {n:4,unit:'weeks',label:'4 semaines'}; const n2=m[2]?Number(m[2]):Number(m[1]); const u=(m[3].startsWith('jour'))?'days':'weeks'; const base=(u==='weeks'?(n2>1?'semaines':'semaine'):(n2>1?'jours':'jour')); return {n:n2,unit:u,label:`${m[1]}${m[2]?`–${m[2]}`:''} ${base}`}; }
...
function renderForm(){
  ...
  defs[calcType.value].fields.forEach((m)=>{
    const wrap=document.createElement('label'); wrap.textContent=m.label;
    let inp;
    if(m.type==='select'){
    ...
    }else if(m.type==='weightPair'){
      const row=document.createElement('div'); row.style.display='grid'; row.style.gridTemplateColumns='1fr 1fr'; row.style.gap='8px';
      const kg=document.createElement('input'); kg.type='number'; kg.id='poidsKg'; kg.placeholder='kg';
      const lb=document.createElement('input'); lb.type='number'; lb.id='poidsLb'; lb.placeholder='lbs';
      kg.addEventListener('input',()=>{ const v=parseFloat(kg.value); if(!isNaN(v)) lb.value=(v/KG_PER_LB).toFixed(2); else lb.value='';});
      lb.addEventListener('input',()=>{ const v=parseFloat(lb.value); if(!isNaN(v)) kg.value=(v*KG_PER_LB).toFixed(2); else kg.value='';});
      row.appendChild(kg); row.appendChild(lb); wrap.appendChild(row);
    }else{
...
function getVals(){
  const vals={};
  defs[calcType.value].fields.forEach((m)=>{
    if(m.type==='concPair'){ vals.conc=Number((document.getElementById('conc')||{}).value||0); vals.concMode=(document.getElementById('concMode')||{}).value||'mg5ml'; return; }
    if(m.type==='weightPair'){ vals.poidsKg=Number((document.getElementById('poidsKg')||{}).value||0); vals.poidsLb=Number((document.getElementById('poidsLb')||{}).value||0); return; }
    const raw=document.getElementById(m.id).value; vals[m.id]=m.type==='number'?Number(raw||0):raw;
  });
  return vals;
}
